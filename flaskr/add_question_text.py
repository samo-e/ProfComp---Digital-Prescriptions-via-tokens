...
try:
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='scenario_questions'")
    if cursor.fetchone():
        print("scenario_questions table still exists (you might want to migrate data first)")
    else:
