...
function selectNew() {
  ...
  checkboxes.forEach(function (cb) {
    ...
    if (statusEl && statusEl.textContent) {
      isAvailable =
        statusEl.textContent.toLowerCase().indexOf("available") !== -1;
    }
...
function updatePatientAvailability() {
  ...
  patientSelects.forEach(function (select) {
    if (select.value) {
      selectedPatients[select.value] =
        (selectedPatients[select.value] || 0) + 1;
    }
  });
...
function assignSelectedStudents() {
  ...
  selectedCheckboxes.forEach(function (checkbox) {
    ...
    if (!patientSelect || !patientSelect.value) {
      ...
      errorMessages.push(
        `${studentName} is selected but has no patient assigned`
      );
    } else {
  ...
  fetch(`/scenarios/${scenarioId}/assign-students`, {
    ...
    .then((data) => {
      if (data.success) {
        alert(
          `Successfully assigned ${data.count} students with their patients!`
        );
...
function setupUnassignButton() {
  ...
  if (unassignBtn) {
    unassignBtn.addEventListener("click", function () {
      ...
      if (
        !confirm(
          "Are you sure you want to unassign the selected students? This will remove their access."
        )
      ) {
        return;
      }
