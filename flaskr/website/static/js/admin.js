    ...
    document.querySelectorAll(".assign-student-form").forEach(function (form) {
      form.addEventListener(
        ...
        function (e) {
          ...
          const studentNameEl = li
            ? li.querySelector("span.student-name")
            : null;
          const studentName = studentNameEl ? studentNameEl.textContent : "";
          const studentId = form.querySelector(
            'input[name="student_id"]'
          ).value;
          const teacherId = form.querySelector(
            ...
            .then((data) => {
              if (data && data.success && data.student) {
                ...
                if (assignedList) {
                  ...
                  if (
                    firstLi &&
                    firstLi.textContent.includes("No students assigned")
                  ) {
                  ...
                  newLi.innerHTML = `
                <span>${
                  data.student.studentnumber ? data.student.studentnumber : ""
      ...
      fetch(form.action, {
        ...
        .then((data) => {
          if (data && data.success) {
            ...
            document
              .querySelectorAll(".assign-student-form")
              .forEach(function (assignForm) {
                const assignStudentId = assignForm.querySelector(
                  'input[name="student_id"]'
                ).value;
