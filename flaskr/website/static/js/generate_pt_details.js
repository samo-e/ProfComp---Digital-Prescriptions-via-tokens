import {
  ...
  en,
  en_AU,
} from "https://cdn.jsdelivr.net/npm/@faker-js/faker@10.0.0/+esm";
...
function generatePtData() {
  ...
  insertIntoField(
    "basic-dob",
    f.date
      .birthdate({ min: 1, max: 90, mode: "age" })
      .toISOString()
      .slice(0, 10)
  );

  // Basic-Contact
  let state = insertIntoField(
    "basic-state",
    ["sa", "vic", "nsw", "qld", "wa", "tas", "nt", "act"][
  ...
  let range =
    postcodeRanges[state][
      Math.floor(Math.random() * postcodeRanges[state].length)
    ];
  ...
  insertIntoField(
    "basic-phone",
    "08" + String(Math.floor(Math.random() * 99999999)).padStart(8, "0")
  );
  insertIntoField(
    "basic-mobile",
    "04" + String(Math.floor(Math.random() * 99999999)).padStart(8, "0")
  );
  insertIntoField(
    "basic-email",
    f.internet.email({ firstName: fname, lastName: lname })
  );

  // Basic-Medicare
  insertIntoField(
    "basic-medicare",
    f.number.int({ min: 100000000, max: 999999999 })
  );
  insertIntoField("basic-medicareIssue", f.number.int({ min: 1, max: 4 }));
