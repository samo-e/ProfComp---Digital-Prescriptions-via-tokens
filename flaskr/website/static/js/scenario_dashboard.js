...
$(document).ready(function () {
  try {
    ...
    const initial =
      $hidden.val() ||
      ($controls.length && $controls.data("exam-start")
        ? "exam"
        : "assignment");
    setMode(initial);
    ...
    try {
      ...
      function setAssignEnabled(enabled) {
        try {
          ...
          if (enabled) {
            ...
            if (!$assignBtn.data("origHref"))
              $assignBtn.data("origHref", $assignBtn.attr("href"));
            try {
      ...
      if (
        ...
        try {
          $assignmentBtn.addClass("disabled");
          $examBtn.addClass("disabled");
        } catch (e) {}
        try {
          if ($startInput && $startInput.length)
            $startInput.prop("disabled", true);
        } catch (e) {}
        try {
          if ($endInput && $endInput.length) $endInput.prop("disabled", true);
        } catch (e) {}
        try {
          if ($hidden && $hidden.length) $hidden.prop("readOnly", true);
        } catch (e) {}
        ...
        if (!$("#inlineEditBtn").length) {
          ...
          if ($("#modeSavedBtn").length) $("#modeSavedBtn").after($edit);
          else if ($saveBtn && $saveBtn.length) $saveBtn.after($edit);
          else $modeForm.append($edit);
          $edit.on("click", function (e) {
            try {
              ...
              try {
                if ($startInput && $startInput.length)
                  $startInput.prop("disabled", false).focus();
              } catch (e) {}
              try {
              ...
              try {
                if ($hidden && $hidden.length) $hidden.prop("readOnly", false);
              } catch (e) {}
              ...
              try {
                let editableClass =
                  $saveBtn && $saveBtn.length && $saveBtn.data("editableClass")
                    ? $saveBtn.data("editableClass")
                    : "btn btn-sm btn-success ms-2";
                let editableHtml =
                  $saveBtn && $saveBtn.length && $saveBtn.data("editableHtml")
                    ? $saveBtn.data("editableHtml")
                    : "Save";
