...
def _build_patient(contract: Dict[str, Any]) -> Patient:
    ...
    patient = Patient(
        ...
        sfty_net_entitlement_cardholder=_to_bool(contract["sfty-net-entitlement-cardholder"]),
        rpbs_ben_entitlement_cardholder=_to_bool(contract["rpbs-ben-entitlement-cardholder"]),
        name=contract["name"],
        dob=_validate_date(contract["dob"], field="dob"),
        preferred_contact=_digits_only_int(contract["preferred-contact"], field="preferred-contact"),
...
def _build_prescription_from_asl(item, patient, prescriber):
    ...
    return Prescription(
        ...
        prescribed_date=_validate_date(item["prescribed-date"], field="prescribed-date"),
...
def ingest_pt_data_contract(pt_data: Dict[str, Any], session, commit=False, overwrite_patient=False) -> IngestResult:
    ...
    if not is_new and overwrite_patient:
        for field in [
            "pharmaceut_ben_entitlement_no","sfty_net_entitlement_cardholder",
            "rpbs_ben_entitlement_cardholder","name","dob","preferred_contact",
            "address_1","address_2","script_date","pbs","rpbs","asl_status",
            "is_registered","consent_last_updated"]:
    ...
    if commit:
        ...
        session.flush()
    return IngestResult(patient, prescribers, prescriptions, created_prescribers, len(prescriptions), is_new)
