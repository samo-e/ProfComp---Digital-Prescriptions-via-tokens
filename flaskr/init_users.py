...
def init_users():
    ...
    with app.app_context():
        ...
        if existing_users:
            print(f"Users already exist in database ({len(existing_users)} users found)")
            response = input("Do you want to delete existing users and create new ones? (y/n): ")
            if response.lower() != 'y':
        ...
        for user_data in test_users:
            user = User(
                ...
                studentnumber=user_data.get('studentnumber')  # Add studentnumber if it exists
            )
