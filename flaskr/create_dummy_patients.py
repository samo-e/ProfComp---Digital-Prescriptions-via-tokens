...
def create_dummy_patients():
    ...
    with app.app_context():
        ...
        repatriation_types = ["Gold Card", "White Card", "Orange Card"]
        ...
        for i in range(10):
            ...
            patient = Patient(
                ...
                pbs=(
                    f"PBS{random.randint(1000, 9999)}"
                    if random.choice([True, False])
                    else None
                ),
                rpbs=(
                    f"RPBS{random.randint(1000, 9999)}"
                    if random.choice([True, False])
                    else None
                ),
                last_name=last_name,
                ...
                concession_number=(
                    f"CN{random.randint(100000, 999999)}"
                    if random.choice([True, False])
                    else None
                ),
                concession_valid_to=(
                    f"{random.randint(1, 12):02d}/{random.randint(current_year, current_year + 2)}"
                    if random.choice([True, False])
                    else None
                ),
                safety_net_number=(
                ...
                repatriation_number=(
                    f"RP{random.randint(100000, 999999)}" if has_repatriation else None
                ),
                repatriation_valid_to=repatriation_valid_to,
                repatriation_type=repatriation_type,
                ndss_number=(
                    f"NDSS{random.randint(10000, 99999)}"
                    if random.choice([True, False])
                    else None
                ),
                ihi_number=(
                    f"{random.randint(800, 899)}{random.randint(1000000000, 9999999999)}"
                    if random.choice([True, False])
                    else None
                ),
