...
def migrate_database():
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='student_scenarios';")
...
def add_missing_columns(cursor):
    ...
    for column_name, column_type in columns_to_add:
        if column_name not in existing_columns:
            try:
                cursor.execute(f"ALTER TABLE student_scenarios ADD COLUMN {column_name} {column_type}")
